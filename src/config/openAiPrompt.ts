...
const modeInstructions: Record<ChatMode, string> = {
	...
	dogmaticSources: 'Answer only from the doctrinal framework associated with Ludwig Ott\'s Fundamentals of Catholic Dogma and Denzinger\'s Sources of Catholic Dogma. If the user asks for other authorities or material beyond those two sources, say this mode is limited to Ott and Denzinger rather than expanding beyond them. Do not cite either work in the final answer, because this mode is prompt-constrained rather than text-verified. If uncertain, say so plainly.',
